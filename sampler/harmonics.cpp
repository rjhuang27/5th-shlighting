...
void Harmonics::Evaluate(const std::vector<Vertex>& vertices)
{
	...
	for (const Vertex& v : vertices)
	{
		...
		for (int i = 0; i < n; i++)
		{
			coefs[i] = coefs[i] + Y[i] * v.color;;		//不是通过此处改变系数的值
		}
	}
	for (Vec3& coef : coefs)
	{
...
vector<float> Harmonics::Basis(const Vec3& pos)
{
	...
	if (degree_ >=4)
	{
		Y[16] = 3.f / 4.f*sqrt(35.f / PI)*x*z*(x*x - z * z);
		Y[17] = 3.f / 4.f*sqrt(35.f /(2.f*PI))*y*z*(3*x*x - z * z);
		Y[18] = 3.f / 4.f*sqrt(5.f / PI)*x*z*(7*y*y - 1.f);
		Y[19] = 3.f / 4.f*sqrt(5.f / (2.f*PI))*y*z*(7*y*y - 3.f);
		Y[20] = 3.f / 16.f*sqrt(1.f / PI)*(35 * y*y*y*y -30*y*y+ 3.f);
		Y[21] = 3.f / 4.f*sqrt(5.f / (2.f*PI))*y*x*(7 * y*y - 3.f);
		Y[22] = 3.f / 8.f*sqrt(5.f / PI)*(x*x-z*z)*(7*y*y-1.f);
		Y[23] = 3.f / 4.f*sqrt(35.f / (2.f*PI))*y*x*(x*x -3* z * z);
		Y[24] = 3.f / 16.f*sqrt(35.f / PI)*(x*x*(x*x-3*z*z)-z*z*(3*x*x-z*z));
	}
	return Y;
}